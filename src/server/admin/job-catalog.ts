...
export const JOB_CATALOG: JobDefinition[] = [
  ...
  },
  {
    ...
    options: [
      { flag: 'library', type: 'library', label: 'Library' },
      { flag: 'limit', type: 'number', label: 'Limit', help: 'Cap videos per run (testing)' },
      { flag: 'reset', type: 'boolean', label: 'Re-render every video' },
    ],
  },
  {
    id: 'enrich:faces',
    ...
    label: 'Enrich — Faces',
    description: 'Detect faces (YuNet) and compute 512-d ArcFace embeddings for photos. Non-frontal and tiny faces are skipped. Required before clustering (enrich:people).',
    category: 'enrich',
    longRunning: true,
    options: [
      { flag: 'library', type: 'library', label: 'Library' },
  ...
  },
  {
    ...
    description: 'Cluster photo face embeddings (512-d ArcFace) into people across all libraries. Run after enrich:faces.',
    category: 'enrich',
    options: [
      { flag: 'threshold', type: 'number', label: 'Threshold', help: 'Euclidean; default 1.0 (= cosine 0.5). Lower = tighter/more clusters' },
      { flag: 'reset', type: 'boolean', label: 'Drop all assignments + people and re-cluster from scratch' },
    ],
  },
